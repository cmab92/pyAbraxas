...
class AbraxasFrame:
    ...
    def trainClassifier(self, classifier, supervised=True):
        ...
        :param supervised: If True use labels.
        :return: writes to:
    ...
    def dumpDataSet(self, dumpName=None):

        """
        Dump data set for convenience...
        :param dumpName: Default "dataSetDump.pkl".
        :return:
        """
        ...
        if isinstance(dumpName, str):
            ...
            dumpName = "dataSetDump.pkl"

        with open(dumpName, 'wb') as dataSetDump:
            cPickle.dump(self.__sourceData, dataSetDump)

    def loadDataSetDump(self, dumpName=None):

        """
        Load dumped data set, window and split to test/training data.
        :param dumpName: Default "dataSetDump.pkl".
        :return: writes to:
         - self.__sourceData
        """
        ...
        with open(dumpName, 'rb') as dataSetDump:
            self.__sourceData = cPickle.load(dataSetDump)

        self.windowSplitSourceDataTT()

        return self.__sourceData

    def dumpTeTrData(self, dumpName=None):

        """
        Dump normalized training and test data features.
        :param dumpName: Default "teTrDataDump.pkl".
        :return:
        """
        ...
        if isinstance(dumpName, str):
            ...
            dumpName = "teTrDataDump.pkl"

        self.initFeatNormalization(inputData=self.__sourceTrainWindowData, dumpName="throwAway.pkl")

        windowFeatures = []
        for i in range(len(self.__sourceTestWindowData)):
            print("(dumpTeTrData) Progress test feature extraction: " +
                  str(100*i/(len(self.__sourceTestWindowData))) + "%")
            windowFeatures.append(self.extractFeatures(self.__sourceTestWindowData[i]))

        self.__sourceTestFeat = self.featureNormalization(features=windowFeatures, initDone=True)

        teTrData = [self.__sourceTrainFeat, self.__sourceTrainLabel, self.__sourceTestFeat, self.__sourceTestLabel]
    ...
    def loadTeTrDump(self, dumpName=None):
        ...
        """
        Load training and test data features.
        :param dumpName: Default "teTrDataDump.pkl".
        :return: writes to:
         - self.self.__sourceTrainFeat
         - self.self.__sourceTrainLabel
         - self.self.__sourceTestFeat
         - self.self.__sourceTestLabel
        """
        ...
        with open(dumpName, 'rb') as teTrDataDump:
            teTrData = cPickle.load(teTrDataDump)

        self.__sourceTrainFeat, self.__sourceTrainLabel, self.__sourceTestFeat, self.__sourceTestLabel = teTrData
